...
const getProfile = async (nickname: string) => {
  if (DATA.NICKNAME !== nickname || DATA.CONTENT === undefined) {
    ...
    await http<IProxy<IUserProfile>>(
      'GET',
      `https://crashmax.ru/api/proxy?url=${BASE_URL}/ajax/mysite/index/${nickname}/`,
      false
    ).then((e) => {
      ...
      if (status === 200) {
        ...
        DATA.CONTENT = e.parsedBody!.contents.content.replace(
