  ...
  try {
    ...
    if (getPath() === '/lenta/readers/' && delLinks && !buttons) {
      ...
      for (let link of delLinks) {
        let chWrap = ce('label', {
          className: 'list-link icon-link sp-ch-readers'
        })
      ...
      if (qs('.sp-ch-readers')) {
            ...
            if (e.target instanceof Element) {
              ...
              for (let ch of checkboxArr) {
                ;(<HTMLInputElement>ch).checked =
                  parent.innerHTML.indexOf('Выбрать все') !== -1 ? true : false
              }
        ...
        const deleteReadersButton = ce('button', {
          ...
          onclick: () => {
            ...
            if (count > 0) {
              modalConfirm(
                `Вы действительно хотите удалить ${count} ${declStr(count)}?`,
                true,
                async () => {
                  ...
                  for (let reader of readers) {
                    modalMessage(
                      `Осталось удалить ${count--} из ${allReaders} ${declStr(count)}`,
                    ...
                    await http(
                      'POST',
                      `${BASE_URL}/lenta/reader_delete/?user=${reader}`,
                      false,
                      `&CK=${DATA.CK}&cfms=Удалить`
                    ).then((e) => {
