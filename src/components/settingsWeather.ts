...
export const settingsWeather = async (e: Element) => {
  ...
  let locationLbl = ce('label', {
    innerHTML:
      'API-Ключ:<div class="label__desc"><a href="https://openweathermap.org/appid" target="_blank">Получить ключ</a></div>',
    className: 'label'
  })
  ...
  let intervalLbl = ce('label', {
    innerHTML:
      'Интервал обновления:<div class="label__desc">от 1 до 360 минут</a></div>',
    className: 'label'
  })
  ...
  let interval = ce('input', {
    ...
    oninput: (e: Event) => {
      ...
      if (
        /^([1-9]|[1-8][0-9]|9[0-9]|[12][0-9]{2}|3[0-5][0-9]|360)$/i.test(value)
      ) {
